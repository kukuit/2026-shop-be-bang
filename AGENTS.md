...
- Bài mới cùng dạng chỉ bổ sung mục tiêu, danh sách/pool câu hỏi, đáp án và lựa chọn, hình và voice qua `GoldMinerGameConfig`/`GoldMinerQuestion`, adapter môn học và `loadQuestions` hiện có. Gắn đúng `learningKey`, `lessonId`, kiểu câu/phương thức nhập-trả lời. Giữ quan hệ đáp án–hình–voice khi random; không sửa engine để thêm nội dung cùng dạng.
