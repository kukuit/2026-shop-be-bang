...
Every task mutation, including one initiated from the tree, is reviewed in chat. Editable fields are validated again when confirmed. Delete/restore use the original proposal instead of accepting edits in the confirmation payload. Proposal status, session ownership, group, parent and expected task version are checked in the same transaction as the task write. Transactions also update a per-user revision to serialize related parent/group changes. Repeated confirms return the saved result without another mutation. Repeated chat/proposal request IDs return the original turn. A stale proposal must be cancelled and recreated.
