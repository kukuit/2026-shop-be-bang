...
The pure resolver in _lib/task-memory.ts uses explicit input > activeDraft > conversationDefaults > overview > system default, with a field allowlist and runtime source metadata. Explicit null clears nullable fields. Hidden groups fall back to an active default group; missing, foreign, deleted or completed/cancelled parents are cleared in preview and rejected if submitted at confirmation. A valid parent determines the group, overriding a conflicting group choice without silently detaching the branch. Parent pickers include available parents across active groups; selecting one sets and disables the group selector. Clearing the parent unlocks the group selector. Duration-derived deadlines are recalculated after start/duration changes; explicit deadlines switch scheduling mode. “Start now” is finalized only at confirmation.
...
- A move changes `parentId` on the moved task and recomputes `rootTaskId`/`depth` for **all descendants, including soft-deleted descendants**, in the same transaction. Descendant versions and update timestamps advance; their content, status and deadlines remain unchanged. All descendants adopt the moved task’s group. Changing a root’s group also updates the entire branch atomically, including soft-deleted descendants; changed descendant versions invalidate stale forms. The server rejects a submitted child group that differs from its parent. Existing inconsistent ancestor chains must be corrected by saving the root before adding beneath them; no bulk data migration runs. Self-parenting and moving into a descendant are rejected before writes. `rootTaskId`/`depth` are server-derived fields, never accepted from clients.
