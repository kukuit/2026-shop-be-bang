...
;(async()=>{
  ...
  try{
    ...
    await click('[aria-controls="overview-toan"]')
    await wait(`document.querySelector('#overview-toan').textContent.includes('Cần luyện thêm')`)
    assert.equal(requests.length,1)
    assert.ok(requests[0].includes('resource=overview') && requests[0].includes('subject=toan'))
    assert.equal(store.reads.length,7)
    assert.equal(await evaluate(`document.querySelector('[aria-controls="overview-tieng-viet"]').textContent`),'Xem thống kê')
    assert.equal(await evaluate(`document.querySelector('[aria-controls="overview-toan"]') === null`),true)
    await new Promise(resolve=>setTimeout(resolve,200));assert.equal(requests.length,1)
    assert.equal(await evaluate('document.documentElement.scrollWidth <= innerWidth'),true)
    fs.writeFileSync(path.join(output,'overview-mobile.png'),Buffer.from((await cdp('Page.captureScreenshot',{format:'png',captureBeyondViewport:false})).data,'base64'))
    await cdp('Emulation.setDeviceMetricsOverride',{width:1280,height:900,deviceScaleFactor:1,mobile:false})
    assert.equal(await evaluate(`new Set(Array.from(document.querySelectorAll('main article')).map(card=>card.getBoundingClientRect().top)).size`),1)
    fs.writeFileSync(path.join(output,'overview-desktop.png'),Buffer.from((await cdp('Page.captureScreenshot',{format:'png',captureBeyondViewport:false})).data,'base64'))
    await cdp('Emulation.setDeviceMetricsOverride',{width:390,height:844,deviceScaleFactor:1,mobile:true})
    requests=[];store.reads.length=0
