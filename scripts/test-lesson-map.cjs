...
test('map API requires login, validates scope and uses the authenticated user only', async () => {
  ...
  assert.equal((await GET(request('grade=lop-3&subject=toan'))).status, 400)
  assert.equal((await GET(request('grade=lop-2&subject=tieng-anh'))).status, 400)
  ...
  assert.deepEqual(calls, [['user-a', 'toan', 'lop-1']])
  ...
  assert.deepEqual(calls.at(-1), ['user-a', 'tieng-viet', 'lop-1'])
  const gradeTwo = await GET(request('grade=lop-2&subject=toan&userId=user-b'))
  assert.equal(gradeTwo.status, 200)
  assert.equal(gradeTwo.body.userId, 'user-a')
  assert.deepEqual(calls.at(-1), ['user-a', 'toan', 'lop-2'])
})

test('grade two uses ten islands and never inherits grade one completion', () => {
  const project = require('./lib/load-project-ts.cjs')()
  const { getProgressLessons } = project('src/components/games/lesson-map/progress-config.ts')
  const gradeTwo = getProgressLessons('toan', 'lop-2')
  assert.equal(gradeTwo.length, 10)
  assert.ok(gradeTwo.every((lesson, index) => lesson.href === `/game/lop-2/toan/bai-${index + 1}`))
  const items = buildLessonMapItems(gradeTwo, games.map(gameId => ({ lessonId: 'toan-1-bai-1', gameId, completedAt: true })))
  assert.equal(items[0].status, 'current')
  assert.equal(items[0].completedGames, 0)
  assert.equal(items[0].totalGames, 4)
  assert.ok(items.slice(1).every(item => item.status === 'locked'))
  const played = buildLessonMapItems(gradeTwo, games.slice(0, 2).map(gameId => ({ lessonId: 'toan-2-bai-1', gameId, completedAt: true })))
  assert.equal(played[0].status, 'completed')
  assert.equal(played[0].stars, 3)
  assert.equal(getProgressLessons('toan').length, 41)
  assert.deepEqual(getProgressLessons('tieng-anh', 'lop-2'), [])
})
