...
;(async () => {
  ...
  try {
    ...
    await clickText('Gấp'); await wait(`document.querySelector('select[aria-label="Ưu tiên"]')`)
    ...
    for (const [text, parentId, title, depth] of [
      ...
      await send(text); await wait(`document.querySelector('.ai-task-form')`); await clickText('Mở rộng'); await wait(`document.querySelector('.ai-task-form select[aria-label="Task cha"]') && !document.querySelector('.ai-task-form select[aria-label="Task cha"]').disabled`)
    ...
    await clickText('Xác nhận lưu'); await wait(`!document.querySelector('dialog[open]')`); assert.equal(await evaluate(`location.pathname`), '/demo/ai-task/tasks')
    ...
    await clickText('Xác nhận lưu'); await wait(`!document.querySelector('dialog[open]')`)
