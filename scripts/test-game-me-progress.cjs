...
test('subject totals are weighted by attempts, current follows lesson order and grade does not leak', () => {
  ...
  assert.equal(empty.totalLessons,10)
  assert.equal(empty.currentLessonId,'toan-2-bai-1')
...
test('overview resumes first unfinished started lesson, then uses the existing next lesson', () => {
  ...
  const gradeTwo = getOverviewLesson(model.buildSubjectProgress('child', 2, 'toan'))
  assert.equal(gradeTwo.lesson.lessonId, 'toan-2-bai-1')
  assert.equal(gradeTwo.lesson.available, true)
  assert.equal(gradeTwo.lesson.games.length, 4)
  const empty = getOverviewLesson(model.buildSubjectProgress('child', 2, 'tieng-viet'))
