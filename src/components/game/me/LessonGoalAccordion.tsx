...
export default function LessonGoalAccordion({ userId, grade, subject, lessonId, isExpanded }: {
  ...
  return <div>
      ...
      {query.data.goals.map(goal => {
        ...
        return <div key={goal.id} className="grid grid-cols-1 items-center gap-3 py-3 md:grid-cols-[minmax(0,1fr)_16rem] md:gap-6">
          <div className="min-w-0 flex-1">
            ...
            <p className="mt-1 text-xs text-slate-500">{goal.recent.attempts ? `Gần đây: ${goal.recent.correct} đúng / ${goal.recent.attempts} lượt` : 'Chưa có lượt trả lời cho mục tiêu này trong 50 phiên gần nhất'}</p>
          ...
          <div className="ml-auto flex w-full max-w-64 flex-col items-end gap-2">
            <span className={`relative isolate inline-flex w-full items-center justify-center overflow-hidden bg-slate-50 px-3 py-2 text-sm font-bold ${progressText}`}>
              <span aria-hidden="true" className={`absolute inset-y-0 left-0 -z-10 ${progressColor}`} style={{ width: `${Math.max(0, Math.min(100, goal.recent.accuracy ?? 0))}%` }} />
              {goal.recent.accuracy === null ? '—' : `${goal.recent.accuracy}% gần đây`}
            </span>
    ...
    <p className="mt-3 text-xs text-slate-500">Mỗi lượt trả lời đều được tính, gồm lần thử lại. Đánh giá dùng tối đa 20 lượt mới nhất của từng mục tiêu trong 50 phiên chơi gần nhất của tài khoản; cần ít nhất 5 lượt. Tiến bộ so với tối đa 20 lượt trước đó, cũng cần ít nhất 5 lượt. Tỷ lệ toàn bộ lịch sử chỉ để tham khảo.</p>
  </div>
}
