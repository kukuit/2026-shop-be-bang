  ...
  return <div className="space-y-5">
    ...
    {query.isError ? <ProgressError retry={() => { void query.refetch() }} /> : !ready || !data ? <ProgressLoading /> : <>
      <section aria-label="Tổng hợp môn học" className="grid gap-3 sm:grid-cols-3">
        ...
        <div className="rounded-2xl border border-slate-200 bg-white p-5"><p className="text-sm text-slate-500">Toàn bộ lịch sử</p><p className="mt-2 text-xl font-black text-slate-900">{data.accuracy === null ? 'Chưa có dữ liệu trả lời' : `${data.accuracy}% đúng`}</p><p className="mt-1 text-xs text-slate-500">{data.attempts > 0 && `${data.correct} đúng / ${data.attempts} lượt · Chỉ để tham khảo`}</p></div>
        <div className="rounded-2xl border border-slate-200 bg-white p-5"><p className="text-sm text-slate-500">Bài tiếp theo</p><p className="mt-2 text-xl font-black text-slate-900">{current ? `Bài ${current.id}` : lessons.length ? 'Đã hoàn thành các bài' : 'Chưa có bài học'}</p><p className="mt-1 text-xs text-slate-500">{current && `${current.title}${current.available ? '' : ' · Sắp có'}`}</p></div>
      </section>
      <p className="text-sm text-slate-500">Hoàn thành tính theo số loại game khác nhau đã chơi xong, không phải số lần chơi hay đánh giá năng lực. Mở từng bài để xem kết quả gần đây và mức tiến bộ theo mục tiêu. Tổng số bài bao gồm cả bài sắp có.</p>
        ...
        {lessons.map(lesson => {
          ...
          return <article key={key} className={`overflow-hidden rounded-2xl border bg-white ${isCurrent ? 'border-blue-300' : 'border-slate-200'}`}>
            <h3><button type="button" aria-expanded={isExpanded} aria-controls={`goals-${lesson.lessonId}`} onClick={() => setExpanded(value => ({ ...value, [key]: !value[key] }))} className="flex w-full items-center gap-3 p-4 text-left hover:bg-slate-50 focus-visible:outline focus-visible:outline-1 focus-visible:-outline-offset-1 focus-visible:outline-blue-500">
              ...
              <span className="min-w-0 flex-1"><span className="block font-bold text-slate-800">Bài {lesson.id} — {lesson.title}</span><span className="mt-1 flex flex-wrap gap-x-3 gap-y-1 text-xs font-normal text-slate-500"><span>{summary?.attempts ? `Đã ghi nhận ${summary.attempts} lượt trả lời` : 'Chưa có lượt trả lời được ghi nhận'}</span><span>{lesson.games.length ? summary?.completionKnown === false ? 'Chưa đủ dữ liệu xác nhận loại game đã hoàn thành' : `${summary?.completedGames ?? 0}/${new Set(lesson.games).size} loại game · Cần ${Math.max(1, Math.min(new Set(lesson.games).size, lesson.requiredGames))} loại để hoàn thành` : 'Chưa có game'}</span><span>{completed ? 'Đã hoàn thành' : !lesson.available ? 'Sắp có' : isCurrent ? 'Bài tiếp theo' : 'Có thể học'}</span></span></span>
