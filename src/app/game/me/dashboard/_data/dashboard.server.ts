...
export async function getRealDashboardData(user: SafeAuthUser): Promise<{
  students: Student[]
  records: StudentRecord[]
}> {
  const { progress: progressRows, sessions } = await getUserGameData(user.id)
  ...
  const records = students.map<StudentRecord>((student) => {
    ...
    return {
      ...
      recentActivities: lessonSessions.slice(0, 5).map((session) => ({
        id: session.id,
