...
const LESSON = { grade: 'Lớp 1', subject: 'Toán', id: 'toan-1-bai-1', label: 'Bài 1 · Nhận biết số 0–5' }
const keys = [0, 1, 2, 3, 4, 5].map((number) => ({ id: `recognize-number-${number}`, label: `Nhận biết số ${number}` }))
const accuracy = (correct: number, attempts: number) => attempts ? Math.round(correct / attempts * 100) : 0

export default async function EvaluationPage({ searchParams }: { searchParams?: { userId?: string } }) {
  const allProgress = await getAdminLearningProgress()
  const lessonProgress = allProgress.filter((item) => item.lessonId === LESSON.id)
  ...
  return <div className="space-y-6">
    <div><p className="text-sm font-bold text-blue-600">{LESSON.grade} / {LESSON.subject}</p><h2 className="text-2xl font-black">{LESSON.label}</h2><p className="mt-1 text-sm text-slate-500">100% nghĩa là chưa có lần trả lời sai; “còn yếu” hiện được hiển thị khi độ chính xác dưới 70%.</p></div>
    <div className="flex flex-wrap gap-2">{lessonProgress.map((item) => <Link key={item.id} href={`/game/admin/danh-gia?userId=${encodeURIComponent(item.userId)}`} className={`rounded-xl px-4 py-2 text-sm font-bold ${item.userId === selectedUserId ? 'bg-blue-600 text-white' : 'border border-slate-200 bg-white text-slate-700'}`}>{item.userId}</Link>)}</div>
    {!selected ? <div className="rounded-2xl border border-dashed border-slate-300 bg-white p-10 text-center text-slate-500">Chưa có dữ liệu tiến độ cho {LESSON.label}.</div> : <>
      ...
      <section className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{keys.map((key) => {
        const progress = selected.keys[key.id] ?? { correct: 0, wrong: 0, attempts: 0, responseTime: 0, sessions: 0 }
